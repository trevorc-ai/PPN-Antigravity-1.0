...
governance_questions_pending: false
governance_answers_received: true
---
...
-- ⚠️  STEP 0: Run this FIRST — confirm CASCADE scope before wiping anything
-- Review output and ensure no unexpected child tables appear.
-- If any table appears that should NOT be wiped, alert SOOP before proceeding.
SELECT
...
WHERE tc.constraint_type = 'FOREIGN KEY'
  AND ccu.table_name IN (
    'log_clinical_records','log_session_vitals','log_red_alerts',
    'log_safety_events','log_behavioral_changes','log_protocols',
    'log_consent','log_patient_flow_events'
  )
ORDER BY parent_table, child_table;

-- ⚠️  Only proceed to TRUNCATE after reviewing the query above.
...
SOOP (schema) → INSPECTOR (QA) → USER (execute SQL) → BUILDER (code fixes) → INSPECTOR (5d audit) → USER (data reset) → ANALYST (seed data)

---

## SECTION 11: GOVERNANCE DECISIONS — LOCKED (2026-02-23T11:00 PST)

> Recorded by SOOP after USER answered all three governance questions.

**Q1 — Free-text narrative columns:**
- 3a `log_clinical_records.session_notes` → **(A)** Stop all writes. UI-ONLY, never persisted.
- 3b `log_clinical_records.contraindication_override_reason` → **(A)** Stop all writes. UI-ONLY.
- 3c `log_behavioral_changes.change_description` → **(A)** Stop all writes. `change_type_ids` ARRAY is correct path.
- 3d `log_behavioral_changes.change_type` → **(A)** Stop all writes. Ghost column. `change_type_ids` ARRAY is correct path.

**Q2 — Wave approach:** ✅ Confirmed — 3-wave structure as scoped in Section 4.

**Q3 — Data reset (TRUNCATE):** ✅ Confirmed — TRUNCATE all 20 clinical log tables listed in Section 9. Keep ref_*, user, site, billing tables intact.

**Additional governance decision (USER):**
- `log_vocabulary_requests` table — **zero FK ties to any site, clinic, or practitioner.** Fully anonymous. Request count only. This feature is separated into its own work order (WO-406 — pending CUE).

---

## SECTION 12: SOOP PRE-FLIGHT CHECKLIST (Required before INSPECTOR handoff)

Per `agent.yaml` line 107: SOOP must append completed pre-flight checklist before routing to INSPECTOR.

**Additive-only compliance:**
- [x] All Wave 1 SQL uses `CREATE TABLE IF NOT EXISTS` only
- [x] All column additions use `ALTER TABLE ... ADD COLUMN IF NOT EXISTS`
- [x] Zero `DROP TABLE`, `DROP COLUMN`, `ALTER COLUMN TYPE` statements
- [x] Zero `TRUNCATE` in migration SQL — TRUNCATE is a separate USER-executed reset script, not a migration

**RLS compliance:**
- [x] All 4 new ref_ tables have `ALTER TABLE ... ENABLE ROW LEVEL SECURITY;` immediately after creation
- [x] RLS SELECT policies added for all 4 tables — `authenticated_read_ref_*` policies written (Fix 1 per INSPECTOR rejection)

**Naming conventions:**
- [x] All new table names: snake_case, lowercase (`ref_session_types`, `ref_crisis_event_types`, etc.)
- [x] All new column names: snake_case, lowercase

**FK reference correctness (verified against live schema map in Section 0):**
- [x] `log_red_alerts.severity_grade_fk` → `ref_severity_grade(severity_grade_id)` ✅ correct PK name
- [x] `log_red_alerts.crisis_event_type_id` → `ref_crisis_event_types(id)` ✅ new table, SERIAL PK named `id`
- [x] `log_system_events.action_type_id` → `ref_system_action_types(id)` ✅ new table, SERIAL PK named `id`
- [x] `log_clinical_records.session_type_id` → `ref_session_types(id)` ✅ new table, SERIAL PK named `id`
- [x] `log_consent.consent_type_id` → `ref_consent_types(id)` ✅ new table, SERIAL PK named `id`

**PHI/PII:**
- [x] No patient identifiers in any migration SQL
- [x] No free-text columns introduced
- [x] All new columns are integer FKs or BOOLEAN or TIMESTAMP

**Idempotency:**
- [x] All INSERTs use `ON CONFLICT (...) DO NOTHING` — safe to re-run
- [x] All CREATE TABLE statements use `IF NOT EXISTS` — safe to re-run
- [x] All ALTER TABLE ADD COLUMN statements use `IF NOT EXISTS` — safe to re-run

**Known open item for INSPECTOR:**
- RLS read policies for the 4 new ref_ tables are not included in Wave 1 SQL.
  If INSPECTOR requires them before approval, SOOP will add:
  `CREATE POLICY "authenticated_read" ON ref_X FOR SELECT TO authenticated USING (true);`
  for each of the 4 new tables.
...
**Date:** 2026-02-23T11:03 PST
**failure_count:** 1

### Required Fixes Before Resubmission:

**Fix 1 — CRITICAL: RLS SELECT policies missing on all 4 new ref_ tables**

`ALTER TABLE ref_X ENABLE ROW LEVEL SECURITY` with no policy = zero rows readable by any user, including authenticated. App forms will silently load empty pickers.

Add to Wave 1 SQL block (before Step 5):
```sql
CREATE POLICY "authenticated_read_ref_session_types"
  ON ref_session_types FOR SELECT TO authenticated USING (true);
...
**Fix 2 — REQUIRED: Add CASCADE pre-verification query to Section 9 before TRUNCATE**

`TRUNCATE ... CASCADE` silently wipes unlisted child tables with FKs pointing to a truncated table. Add this pre-flight query so USER confirms scope before executing:
```sql
-- Run BEFORE the TRUNCATE — confirm no unexpected child tables in scope
SELECT
    tc.table_name AS child_table,
    ccu.table_name AS parent_table,
    kcu.column_name AS fk_column
...
**Fix 3 — Git push required before final PASS**
`git log` shows `HEAD -> main` with no `origin/main` tag. Push to remote required before INSPECTOR can issue final [STATUS: PASS].

### Audit Items That PASSED (do not rework):
- ✅ Additive-only SQL — no DROP, ALTER TYPE, or TRUNCATE in migration
- ✅ All 4 tables snake_case lowercase
- ✅ No PHI/PII columns introduced
- ✅ No free-text columns introduced
- ✅ All INSERTs idempotent via ON CONFLICT DO NOTHING
- ✅ FK references verified against live schema map in Section 0
