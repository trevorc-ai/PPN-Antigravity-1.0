...
- [ ] **Route Check:** Does `/journey/auth` exist as a `<Route>` in `App.tsx`? (Expected: NO — this is the bug)
- [ ] **Live Redirect Confirmed:** Does visiting the magic link URL on ppnportal.net redirect to Dashboard? (Expected: YES — confirm bug is real)
- [ ] **Token Format:** Is the generated link format correct per MagicLinkModal.tsx? (Expected: YES — link generation is fine)
- [ ] **Verdict:** Route is missing. Reject and route to LEAD for architecture decision on the `/journey/auth` handler.
...
- [ ] Visit `ppnportal.net/internal/trevor-showcase.html` — confirm page loads (200 OK)
- [ ] Check response headers — confirm `X-Robots-Tag: noindex, nofollow` is present
- [ ] Visit `ppnportal.net/robots.txt` — confirm `Disallow: /internal/` appears and old individual disallows are gone

**VIP invite tool functionality:**
- [ ] Visit `ppnportal.net/internal/vip-invite-flow.html` on mobile (iOS Safari preferred)
- [ ] Tap through all 4 tabs (Your Tool / Their Message / They Land / They Enter)
- [ ] On Step 1 (Admin Tool): paste a test URL into the Partner card input, tap Share — confirm Web Share API fires (iOS) or clipboard fallback triggers (desktop)
- [ ] On Step 3 (Beta Welcome): tap "Enter the Network →" — confirm tab navigation to Step 4 works
- [ ] Font size check: no text below 14px
- [ ] Em dash check: no em dashes in any copy

**`vercel.json` correctness:**
- [ ] Confirm `headers` block appears before `rewrites` block in `vercel.json`
- [ ] No syntax errors in JSON

### Inspector checklist — Item 2
- [ ] `X-Robots-Tag: noindex, nofollow` confirmed on `/internal/*` paths on live site
- [ ] `robots.txt` shows `Disallow: /internal/` (single entry, clean)
- [ ] `vip-invite-flow.html` loads and all 4 tabs navigate correctly
- [ ] Share button fires native share sheet on mobile / copies to clipboard on desktop
- [ ] No typography violations (no text below 14px, no em dashes)
- [ ] `vercel.json` is valid JSON with no syntax errors
- [ ] No React source files modified (HTML-only changes)
...
| VIP invite flow + restructure | 🟡 Pending Inspector QA | None if QA passes — approve for commit |
