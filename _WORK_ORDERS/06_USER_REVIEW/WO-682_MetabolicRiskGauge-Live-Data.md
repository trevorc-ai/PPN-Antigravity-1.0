...
---
- **Data from:** `log_session_vitals` (most recent vitals per active patient — HR, BP); intake risk factors (if available in `log_clinical_records`)
- **Data to:** No DB writes — `MetabolicRiskGauge.tsx` gauge display only (read-only); honest zero-state if no vitals data
- **Theme:** Tailwind CSS, Recharts gauge — `MetabolicRiskGauge.tsx`; PPN design system; no synthetic values permitted
