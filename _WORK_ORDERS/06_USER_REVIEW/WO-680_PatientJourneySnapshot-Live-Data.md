...
---
- **Data from:** `log_clinical_records` (session data), `log_session_vitals` (vital timestamps), `log_safety_events` (AE/rescue/note events joined on `session_id`); substance protocol config (dose timing windows)
- **Data to:** No DB writes — `PatientJourneySnapshot.tsx` timeline display only (read-only)
- **Theme:** Tailwind CSS, Recharts/custom timeline — `PatientJourneySnapshot.tsx`; "Protocol-estimated" phase band label required on all phase markers
