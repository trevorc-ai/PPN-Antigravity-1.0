...
**Owner:** TREVOR (manual Supabase dashboard action)
**Route:** 01_TRIAGE → 05_USER_REVIEW

### Routing Decision:
This is a Trevor-only action — no code changes required, no BUILDER or SOOP involvement. The checklist in the ticket is complete and actionable.

**Trevor must complete the pre-deletion audit first (run the 3 SQL queries in the old project) before deleting anything.**

After deletion, Trevor updates the `.env.local` header comment — no BUILDER handoff needed for that minor text change.
