...
### 6. LEAD Decisions (2026-03-09)

| # | Question | LEAD Decision |
|---|----------|---------------|
| 1 | Is the hamburger button currently rendering on mobile? | **Yes — it is rendered.** `HelpCenterLayout.tsx` line 73 has the toggle button wired to `mobileMenuOpen`. The sidebar is `hidden lg:block` by default and the button toggles it. BUILDER: **do not add a new button** — only verify it renders above the fold on 375px. |
| 2 | Rename "Session Reporting" sidebar label to "Downloads & Exports"? | **No — keep the existing label.** Renaming now would confuse practitioners who already know it. The content update (directing to `/download-center`) is in scope; the label is not. |
| 3 | Features not yet live (Trial Matchmaker, Music Logger, Legacy Importer) — remove, keep, or Coming Soon? | **Flag as Coming Soon.** Add a `badge="Coming Soon"` inline label next to the sidebar link and a one-line note at the top of the relevant subpage section. Do not remove content — it signals the roadmap. |
