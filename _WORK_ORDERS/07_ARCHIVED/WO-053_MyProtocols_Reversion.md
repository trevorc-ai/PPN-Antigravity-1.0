...
**Instructions:** Replace the current page with the old version 'MyProtocols_plus_Modal.tsx' and remove the modal completely. We are building a new and improved version of the ProtocolBuilder modal called 'Wellness Journey' (working title), which we will connect to the 'Create New Protocol' button. Only add the page, connect the display to the log tables in the database, and update to match the current CSS; do not add the modal.
...
Revert the My Protocols page to the previous design (without the modal) and prepare it for the new "Wellness Journey" Protocol Builder feature.
...
- **NO modal** - the old modal will be replaced with a new "Wellness Journey" feature
...
- Connect "Create New Protocol" button to future "Wellness Journey" feature (placeholder for now)
...
5. **Connect "Create New Protocol" button** to placeholder route (for future Wellness Journey)
...
- Build the "Wellness Journey" feature yet (placeholder only)
...
<button
  ...
  // OR
  onClick={() => alert('Wellness Journey feature coming soon!')}
...
**Future Work:**
- Wellness Journey Protocol Builder (separate work order)

---

## Notes

This is a **reversion + cleanup** task. The goal is to restore the previous design that the user prefers, remove the modal that's no longer needed, and prepare the page for the upcoming "Wellness Journey" feature.

The "Wellness Journey" is a new Protocol Builder modal/feature that will be built separately and connected to the "Create New Protocol" button in a future work order.
