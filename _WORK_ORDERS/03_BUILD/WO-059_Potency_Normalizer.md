...
1. **Critical Missing Artifacts:** Database migration file `migrations/040_add_substance_batches.sql` is MISSING. The application relies on `substance_batches` and `ref_substance_strains` tables which do not exist.
2. **Frontend Mocking:** The `PotencyNormalizerCard` component uses a hardcoded `STRAIN_DATABASE` and local state instead of fetching from/saving to the database. This fails Acceptance Criteria: "Batch data saves to database" and "User can select from previously saved batches".
3. **Action Required:**
    - **SOOP:** Create the `040_add_substance_batches.sql` migration with the specified schema and RLS policies. Verify idempotency.
    - **SOOP:** Once migration is ready, update owner to `BUILDER`.
    - **BUILDER:** Update `PotencyNormalizerCard` to use Supabase client for real data persistence instead of mocks.
