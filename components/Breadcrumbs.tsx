  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2.5">
        <Link
          to="/advanced-search"
          className={`flex items-center gap-2 text-[12px] font-black tracking-[0.2em] transition-all group ${isPortalActive ? 'text-primary' : 'text-slate-500 hover:text-primary'
            }`}
        >
        ...
        {pathnames.map((value, index) => {
          ...
          return (
            <React.Fragment key={to}>
              ...
              {last ? (
                <span className="text-[12px] font-black text-slate-300 tracking-widest animate-in fade-in duration-300">
                  {label}
                </span>
              ) : (
                <Link
                  to={to}
                  className="text-[12px] font-black text-slate-400 tracking-widest hover:text-slate-200 transition-colors"
                >
                  {label}
                </Link>
              )}
        ...
        {hash && labels[hash] && (
          <>
            <span className="material-symbols-outlined text-slate-700 text-[15px] select-none opacity-50">chevron_right</span>
            <span className="text-[12px] font-black text-slate-300 tracking-widest animate-in fade-in slide-in-from-left-1 duration-300">
              {labels[hash]}
            </span>
          </>
        )}
