        ...
        <div className="w-full bg-[#0f1218] p-6 rounded-2xl border border-slate-800 shadow-2xl flex flex-col gap-8 h-full">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 flex-1">
                ...
                <div className="lg:col-span-2 bg-slate-900/30 border border-slate-800/50 rounded-2xl p-6 relative min-h-[300px] flex flex-col">
                    <h3 className="text-[10px] font-bold text-slate-500 uppercase tracking-widest absolute top-4 left-4 z-10">
                                        ...
                                        if (active && payload && payload.length) {
                                            return (
                                                <div className="bg-slate-900 border border-slate-700 p-3 rounded-lg shadow-xl z-50">
                                                    <p className="text-xs font-bold text-white mb-1">{payload[0].payload.receptor}</p>
                                                    <p className="text-[10px] text-indigo-400 uppercase tracking-widest">{payload[0].payload.role}</p>
                                                </div>
                                            );
                                        }
                                        return null;
                ...
                <div className="flex flex-col gap-4">
                    ...
                    <div className="p-5 bg-gradient-to-br from-slate-900 to-slate-900/50 border border-slate-700 rounded-2xl relative overflow-hidden group">
                        ...
                        <h3 className="text-2xl font-black text-white tracking-tight mb-1 relative z-10">{data.name}</h3>
                        ...
                        <div className="space-y-3 relative z-10">
                            <div className="flex items-center justify-between p-2 bg-black/20 rounded-lg border border-white/5">
                                <span className="text-[10px] text-slate-500 font-bold uppercase">Class</span>
                                <span className="text-xs font-medium text-slate-300">{data.class}</span>
                            </div>
                            <div className="flex items-center justify-between p-2 bg-black/20 rounded-lg border border-white/5">
                                <span className="text-[10px] text-slate-500 font-bold uppercase">Half-Life</span>
