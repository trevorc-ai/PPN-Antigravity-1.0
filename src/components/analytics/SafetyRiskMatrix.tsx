    ...
    <div className="bg-[#0a0c10] border border-slate-800 rounded-[2.5rem] p-6 sm:p-8 shadow-xl flex flex-col h-full relative overflow-hidden group">
      ...
      <div className="flex justify-between items-start mb-6 relative z-10">
        <div className="space-y-1">
          ...
          <p className="text-sm font-bold text-slate-500 uppercase tracking-widest ml-1">Cross-Interaction Risk Map</p>
        </div>
        ...
        <div className="grid grid-cols-6 gap-2 sm:gap-3">
          ...
          {SUBSTANCES.map((sub) => (
            <div key={sub} className="flex items-end justify-center pb-2">
              <span className="text-xs font-black text-slate-300 uppercase tracking-widest -rotate-45 sm:rotate-0 origin-bottom-left sm:origin-center translate-y-2 sm:translate-y-0 whitespace-nowrap">
                {sub.split('-')[0]}
              </span>
            </div>
          ))}
          ...
          {MED_CLASSES.map((med) => (
            <React.Fragment key={med}>
              ...
              <div className="col-span-1 flex items-center justify-end pr-3">
                <span className="text-xs font-black text-slate-500 uppercase tracking-widest text-right leading-tight">
                  {med}
                </span>
              </div>
      ...
      <div className="mt-4 pt-4 border-t border-slate-800 flex justify-center gap-6">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-black text-slate-500 uppercase tracking-widest">Contraindicated</span>
        </div>
        <div className="flex items-center gap-2">
          <div className="size-2 rounded-full bg-orange-500"></div>
          <span className="text-xs font-black text-slate-500 uppercase tracking-widest">High Risk</span>
        </div>
        <div className="flex items-center gap-2">
          <div className="size-2 rounded-full bg-slate-700"></div>
          <span className="text-xs font-black text-slate-500 uppercase tracking-widest">Safe / No Data</span>
        </div>
      </div>
