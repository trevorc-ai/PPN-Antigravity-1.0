    ...
    if (isNode) {
       return (
        <div className="bg-[#0f172a] border border-slate-700 p-3 rounded-xl shadow-2xl backdrop-blur-md z-50">
          <p className="text-sm font-black text-slate-500 uppercase tracking-widest mb-1">Clinical Stage</p>
          <span className="text-xs font-bold text-slate-300">{data.payload.name}</span>
          <div className="mt-2 pt-2 border-t border-slate-800">
             <span className="text-xs text-slate-300">Total Volume: {data.value}</span>
          </div>
        </div>
       );
    }
    ...
    return (
      <div className="bg-[#0f172a] border border-slate-700 p-3 rounded-xl shadow-2xl backdrop-blur-md z-50">
        <div className="flex items-center gap-2 mb-2 border-b border-slate-700/50 pb-2">
           <span className="text-xs font-black text-slate-300 uppercase tracking-widest">{sourceName} → {targetName}</span>
        </div>
        <div className="flex justify-between gap-4">
          <span className="text-xs text-slate-500 font-bold uppercase tracking-wide">Retention:</span>
          <span className="text-xs text-emerald-400 font-mono font-black">{value} Patients</span>
        </div>
      </div>
    );
  }
    ...
    <div className="bg-[#0a0c10] border border-slate-800 rounded-[2.5rem] p-6 sm:p-8 shadow-xl flex flex-col h-full relative overflow-hidden group">
      ...
      <div className="flex justify-between items-start mb-6 relative z-10">
        <div className="space-y-1">
          ...
          <p className="text-sm font-bold text-slate-500 uppercase tracking-widest ml-1">Integration Gap Analysis</p>
        </div>
