...
const NavIconButton: React.FC<{
  ...
  <div className="relative group/tooltip flex flex-col items-center gap-1">
    ...
    <div className="absolute -bottom-10 left-1/2 -translate-x-1/2 px-2.5 py-1.5 bg-[#0c0f16] border border-white/10 rounded-lg shadow-2xl opacity-0 invisible group-hover/tooltip:opacity-100 group-hover/tooltip:visible transition-all duration-200 z-[100] whitespace-nowrap pointer-events-none scale-90 group-hover/tooltip:scale-100">
      ...
      <span className="text-sm font-black text-slate-300 tracking-[0.15em] relative z-10">{tooltip}</span>
    </div>
  </div>
);
            ...
            <div className="hidden lg:flex items-center gap-10">
              {[
                ...
                <button
                  ...
                  className="text-sm font-black text-slate-300 hover:text-slate-300 tracking-[0.25em] transition-all relative group"
                >
                  {link.label}
        ...
        <div className="flex items-center gap-6">
          {isAuthenticated ? (
            <>
                ...
                <div className="hidden lg:flex items-center gap-2 mr-2">
                  {activeChips.slice(0, 3).map(chip => (
                    <button
                      ...
                      className="flex items-center gap-1.5 px-3 py-1.5 bg-emerald-500/10 border border-emerald-500/30 hover:border-emerald-400/60 hover:bg-emerald-500/20 rounded-xl transition-all text-sm font-bold text-emerald-300 active:scale-95"
                  ...
                  {activeChips.length > 3 && (
                    <span className="px-2 py-1 bg-slate-800/60 border border-slate-700/50 rounded-lg text-sm text-slate-400 font-bold">
                      +{activeChips.length - 3}
                    </span>
                  )}
                </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="hidden lg:block relative group/tooltip flex flex-col items-center gap-1">
                  ...
                  <div className="absolute -bottom-10 left-1/2 -translate-x-1/2 px-2.5 py-1.5 bg-[#0c0f16] border border-white/10 rounded-lg shadow-2xl opacity-0 invisible group-hover/tooltip:opacity-100 group-hover/tooltip:visible transition-all duration-200 z-[100] whitespace-nowrap pointer-events-none scale-90 group-hover/tooltip:scale-100">
                    <div className="absolute -top-1 left-1/2 -translate-x-1/2 size-2 bg-[#0c0f16] border-t border-l border-white/10 rotate-45"></div>
                    <span className="text-sm font-black text-teal-300 tracking-[0.15em] relative z-10">Leave Feedback</span>
                  </div>
              ...
              <div id="tour-user-profile" className="relative group/avatar" ref={menuRef}>
                ...
                >
                  ...
                  <div className="hidden lg:flex flex-col">
                    <p className="text-sm font-black text-slate-300 leading-none mb-1 group-hover:text-primary transition-colors">
                      {loading ? 'Loading...' : (userProfile?.display_name || userProfile?.email?.split('@')[0] || 'User')}
                    </p>
                    <div className="flex items-center gap-1">
                      <span className="text-sm text-slate-300 font-bold tracking-widest leading-none">Practitioner</span>
                ...
                {isMenuOpen && (
                  <div className="absolute right-0 mt-3 w-56 bg-[#0c0f16] border border-white/10 rounded-2xl shadow-2xl py-2 z-50 animate-in fade-in slide-in-from-top-2 duration-200 backdrop-blur-3xl">
                    <div className="px-4 py-3 border-b border-white/5 mb-2">
                      <p className="text-sm font-black text-slate-300 tracking-widest leading-none mb-1">Signed In As</p>
            ...
            <button
              ...
              className="px-6 py-2.5 bg-indigo-600 hover:bg-indigo-500 hover:bg-blue-600 text-white text-sm font-black rounded-2xl tracking-widest transition-all shadow-[0_0_15px_rgba(43,116,243,0.3)] active:scale-95"
            >
              Login
            </button>
          )}
