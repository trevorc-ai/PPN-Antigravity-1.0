...
import { Brain, Shield, CheckCircle, FileText, ClipboardCheck, HeartPulse, User } from 'lucide-react';
...
const Phase1HUD: React.FC<Phase1HUDProps> = ({ journey, gates, contraindicationResult }) => {
    ...
    return (
        ...
        >
            ...
            {/* ── Patient Profile chip layout (Substance/Dosage removed — Phase 2 items) ── */}
...
export const PreparationPhase: React.FC<PreparationPhaseProps> = ({ journey, onOpenForm, medications = [], onProceedToPhase2 }) => {
    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500" role="main" aria-label="Phase 1: Preparation">
            ...
            {/* Proceed to Phase 2 — unlocks when all 4 gates are complete.
                 Platform principle: warn but never block. Practitioners decide.
                 RiskEligibilityReport removed from Phase 1 — substance not selected yet.
                 Contraindication screening belongs in Phase 2 after dosing protocol is set. */}
            {allGatesPassed && (
                <div className="mt-8 pt-6 border-t border-slate-700/50">
                    <button
                        id="preparation-proceed-phase2"
                        onClick={onProceedToPhase2}
                        className="w-full py-4 rounded-2xl text-base sm:text-lg font-black uppercase tracking-widest transition-all shadow-xl flex items-center justify-center gap-3 bg-emerald-600 hover:bg-emerald-500 text-white shadow-emerald-900/40 hover:-translate-y-1"
                    >
