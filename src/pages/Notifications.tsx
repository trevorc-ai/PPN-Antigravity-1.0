      ...
      <Section spacing="default" className="flex flex-col lg:flex-row gap-10">
        ...
        <div className="flex-1 space-y-8">
          ...
          <div className="flex items-center gap-1 bg-[#111418] p-1 rounded-xl w-fit border border-slate-800">
            {['All', 'Unread', 'Flagged'].map((tab) => (
              <button
                ...
                className={`px-6 py-2 rounded-lg text-xs font-black uppercase tracking-widest transition-all flex items-center gap-2 ${activeTab === tab
                  ? 'bg-accent-amber text-black shadow-lg shadow-accent-amber/10'
                  : 'text-slate-500 hover:text-slate-300'
                  }`}
              >
                {tab}
                {tab === 'Unread' && <span className="px-1.5 py-0.5 bg-accent-amber/20 text-accent-amber rounded text-xs">3</span>}
              </button>
            ))}
          </div>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-start mb-1">
                      ...
                      <div className="flex items-center gap-3">
                        <span className="text-xs font-mono text-slate-500">{notif.timestamp}</span>
                        {notif.isUnread && (
                          <div className="size-2 bg-accent-amber rounded-full shadow-[0_0_8px_#f59e0b]"></div>
                        )}
                        {notif.type === 'critical' && (
                          <span className="px-2 py-0.5 bg-accent-amber/10 text-accent-amber text-xs font-black uppercase rounded border border-accent-amber/20">Urgent</span>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-24">
                      <div className="flex items-center gap-6">
                        {notif.metadata.map((meta, i) => (
                          <div key={i} className="flex items-center gap-2">
                            ...
                            <span className="text-xs text-slate-500 font-bold uppercase tracking-widest">{meta.label}:</span>
                            <span className="text-xs text-slate-300 font-bold">{meta.value}</span>
                          </div>
                        ))}
                      </div>
                      ...
                      {notif.actionLabel && (
                        <button
                          ...
                          className="flex items-center gap-2 text-accent-amber hover:text-slate-300 text-xs font-black uppercase tracking-[0.2em] transition-all ml-auto"
                        >
                          {notif.actionLabel}
          ...
          <div className="flex justify-center pt-4">
            <button className="text-xs font-black text-slate-600 uppercase tracking-[0.3em] hover:text-slate-300 transition-colors">Load older notifications</button>
          </div>
        </div>
        ...
        <div className="w-full lg:w-[380px] space-y-6">
          <div className="bg-[#111418] border border-slate-800 rounded-[2rem] p-8 shadow-2xl">
            ...
            <div className="space-y-8">
              <div className="space-y-4">
                <h3 className="text-xs font-black text-slate-600 uppercase tracking-[0.3em]">Channel Settings</h3>
                ...
                {[
                  ...
                  <div key={pref.id} className="flex items-center justify-between">
                    <div>
                      <p className="text-sm font-bold text-slate-300">{pref.label}</p>
                      <p className="text-sm text-slate-500">{pref.sub}</p>
                    </div>
                    <button
              ...
              <div className="space-y-4 pt-4 border-t border-slate-800/50">
                <h3 className="text-xs font-black text-slate-600 uppercase tracking-[0.3em]">Delivery Methods</h3>
                <div className="flex gap-6">
                  <label className="flex items-center gap-2 cursor-pointer group">
                    ...
                    />
                    <span className="text-xs font-black text-slate-300 group-hover:text-slate-300 uppercase tracking-widest">Email</span>
                  </label>
                  <label className="flex items-center gap-2 cursor-pointer group">
                    <input
                      type="checkbox"
                      ...
                      className="size-4 bg-slate-800 border-slate-700 rounded text-accent-amber focus:ring-accent-amber/40"
                    />
                    <span className="text-xs font-black text-slate-300 group-hover:text-slate-300 uppercase tracking-widest">Push</span>
                  </label>
                </div>
              </div>
            </div>
          ...
          <div className="bg-[#111418] border border-slate-800 rounded-[2rem] p-8 relative overflow-hidden group shadow-2xl">
            ...
            <div className="relative z-10 space-y-4">
              ...
              <button className="w-full sm:w-auto px-6 py-3 bg-slate-800 hover:bg-slate-700 border border-slate-700 text-xs font-black text-slate-300 uppercase tracking-widest rounded-xl transition-all">
                Contact Support
              </button>
            </div>
          </div>
