...
const AuditLogs: React.FC = () => {
  ...
  return (
    <div className="p-6 sm:p-10 h-full flex flex-col gap-8 animate-in fade-in duration-500 bg-[#020408]">
                ...
                <tr key={log.id} className="hover:bg-white/[0.03] transition-all group border-b border-slate-800/30">
                  ...
                  <td className="px-10 py-6">
                    <div className="flex flex-col gap-1">
                      ...
                      <span className="text-xs text-slate-700 font-mono tracking-tighter uppercase">ID: {log.id}</span>
                    </div>
                  </td>
                  <td className="px-10 py-6">
                    <div className="flex flex-col gap-1">
                      ...
                      <span className="text-xs text-slate-600 font-medium italic tracking-tight">
                        "{log.details}"
                      </span>
                    </div>
                  </td>
                  <td className="px-10 py-6">
                    <div className="flex items-center gap-3">
                      <div className={`size-3 rounded-full ${
                        (log.status === 'AUTHORIZED' || log.status === 'VERIFIED') ? 'bg-emerald-500 shadow-[0_0_10px_#10b981]' : 
                        log.status === 'ALERT_TRIGGERED' ? 'bg-rose-500 shadow-[0_0_10px_#f43f5e]' : 
                        'bg-primary shadow-[0_0_10px_#2b74f3]'
                      }`}></div>
                      <span className={`text-xs font-black uppercase tracking-widest ${
                        (log.status === 'AUTHORIZED' || log.status === 'VERIFIED') ? 'text-emerald-500' : 
        ...
        <div className="px-10 py-8 border-t border-slate-800 bg-black/20 flex flex-col sm:flex-row justify-between items-center gap-6">
          <div className="flex items-center gap-6">
             ...
             <div className="flex items-center gap-3">
                ...
                <span className="text-xs font-mono text-slate-500 uppercase tracking-widest">Global Uplink: Stable</span>
             </div>
          </div>
