          ...
          <div className="grid grid-cols-4 gap-6">
            {categories.map((cat, idx) => (
              <div 
                ...
                className={`bg-[#0D121C] border p-8 rounded-xl transition-all cursor-pointer group shadow-sm ${
              ...
              >
                <div className={`w-12 h-12 rounded-lg flex items-center justify-center mb-6 transition-colors ${
                  selectedCategory === cat.title ? 'bg-primary/20' : 'bg-primary/10 group-hover:bg-primary/20'
                }`}>
                  {cat.icon}
                </div>
                  ...
                  <div key={idx} className="border border-slate-800/80 rounded-xl bg-[#0D121C] overflow-hidden shadow-sm animate-in fade-in slide-in-from-bottom-2 duration-300">
                    ...
                    {activeFAQ === idx && (
                      <div className="px-8 pb-6 text-sm text-slate-400 leading-relaxed border-t border-slate-800/40 pt-4">
                        <div className="mb-3">
                          <span className="text-[10px] font-black text-primary uppercase tracking-widest px-2 py-0.5 bg-primary/10 rounded border border-primary/20">
                            {faq.category}
                          </span>
                        </div>
                        {faq.a}
            ...
            <div className="bg-[#0D121C] border border-slate-800 p-6 rounded-xl flex items-center gap-5 shadow-sm">
              ...
              <div>
                ...
                <div className="text-xs text-slate-500 mt-1">
                  Database latency: <span className="text-slate-400">24ms</span> • Last updated: <span className="text-slate-400">2 mins ago</span>
                </div>
              </div>
            </div>
          </div>
