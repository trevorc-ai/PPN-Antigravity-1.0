...
const IngestionHub: React.FC = () => {
  ...
  return (
    <div className="p-6 sm:p-10 space-y-10 animate-in fade-in duration-700 max-w-[1600px] mx-auto pb-24">
      {/* Header Section */}
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        <div className="space-y-1">
          <div className="flex items-center gap-3 text-primary">
            <span className="material-symbols-outlined text-4xl">upload_file</span>
            <h1 className="text-3xl sm:text-4xl font-black tracking-tighter text-slate-100">Registry Upload</h1>
          </div>
          <p className="text-slate-500 text-[10px] font-black uppercase tracking-[0.3em] ml-1">Institutional Data Contribution // Registry Node 0x7</p>
        </div>
        ...
        <div className="flex items-center gap-3">
          <div className="px-4 py-2 bg-slate-900/60 border border-slate-800 rounded-xl flex items-center gap-3 shadow-xl backdrop-blur-md">
            ...
            <span className="text-[10px] font-mono font-bold text-slate-400 uppercase tracking-widest">Uplink: Nominal</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <section className="bg-black border border-slate-800 rounded-[2.5rem] p-8 sm:p-10 shadow-2xl relative overflow-hidden group">
          ...
          <div className="space-y-6 relative z-10">
            <div className="space-y-1">
              <h2 className="text-2xl font-black text-white tracking-tight">Legacy Chart OCR</h2>
              <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Digital extraction from physical clinical records</p>
            </div>

            <div 
              onDragOver={(e) => { e.preventDefault(); setIsDragging(true); }}
              onDragLeave={() => setIsDragging(false)}
              className={`min-h-[280px] border-2 border-dashed rounded-[2rem] flex flex-col items-center justify-center transition-all duration-300 gap-4 ${
            ...
            >
              ...
              <div className="text-center">
                ...
                <p className="text-[10px] text-slate-600 font-bold uppercase mt-1">or click to browse local archive</p>
              </div>
              <button className="px-6 py-2.5 bg-slate-800 hover:bg-slate-700 text-[10px] font-black text-white rounded-xl uppercase tracking-widest transition-all">
                Select Files
              </button>
            </div>
        ...
        <section className="bg-black border border-slate-800 rounded-[2.5rem] p-8 sm:p-10 shadow-2xl relative overflow-hidden group">
          ...
          <div className="space-y-8 relative z-10 h-full flex flex-col">
            <div className="space-y-1">
              <h2 className="text-2xl font-black text-white tracking-tight">Neural Dictation</h2>
              <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Real-time voice-to-protocol synthesis</p>
            </div>
            ...
            <div className="flex-1 flex flex-col items-center justify-center space-y-10 py-4">
              <div className="relative">
                ...
                <button 
                  ...
                  className={`size-32 rounded-full border-4 flex items-center justify-center transition-all duration-500 relative z-10 shadow-2xl ${
                    isRecording 
              ...
              <div className="text-center space-y-2">
                ...
                <p className="text-[10px] text-slate-600 font-bold uppercase tracking-[0.3em]">Institutional HIPAA Buffer Active</p>
              </div>
            </div>

            <div className="flex gap-3">
              <button className="flex-1 py-4 bg-slate-900 border border-slate-800 rounded-2xl text-[10px] font-black text-slate-400 uppercase tracking-widest transition-all hover:text-white">
                Input Settings
              </button>
              <button className="flex-1 py-4 bg-primary hover:bg-blue-600 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-xl shadow-primary/10 transition-all active:scale-95">
                Review Lexicon
              </button>
            </div>
          </div>
      ...
      <section className="bg-black border border-slate-800 rounded-[2.5rem] overflow-hidden shadow-2xl backdrop-blur-xl">
        <div className="px-8 py-6 border-b border-slate-800 flex items-center justify-between bg-slate-900/20">
          <div className="flex items-center gap-3">
            <span className="material-symbols-outlined text-primary">analytics</span>
            <h3 className="text-base font-black text-white tracking-tight">Upload Queue</h3>
          </div>
          <span className="text-[10px] font-mono text-slate-500 uppercase tracking-widest">Sync_ID: 0x77AF</span>
        </div>
              ...
              {pendingJobs.map((job) => (
                <tr key={job.id} className="hover:bg-primary/5 transition-colors group">
                  <td className="px-8 py-5">
                    <span className="text-[11px] font-mono font-black text-white tracking-tight">{job.id}</span>
                  </td>
                  <td className="px-8 py-5">
                    <span className="text-[11px] font-bold text-slate-400 group-hover:text-slate-200 transition-colors uppercase">{job.source}</span>
                  </td>
                  <td className="px-8 py-5">
                  ...
                  <td className="px-8 py-5">
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`text-[10px] font-black uppercase tracking-tighter ${job.status === 'Processing' ? 'text-primary' : 'text-slate-500'}`}>{job.status}</span>
                    </div>
                  </td>
                  <td className="px-8 py-5 text-right">
                    <span className="text-[10px] font-mono text-slate-600">{job.time}</span>
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        <div className="px-8 py-6 bg-slate-900/10 border-t border-slate-800 flex justify-between items-center">
          ...
          <button className="text-[10px] font-black text-primary hover:text-white uppercase tracking-widest transition-colors flex items-center gap-2">
            View All Jobs
